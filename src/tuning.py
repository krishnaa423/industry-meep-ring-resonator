...
from spectrum import BaselineRunSpec, run_baseline_simulation
...
BASELINE_SUMMARY_PATH = REPORTS_DIR / "run_summary.json"
BASELINE_CSV_PATH = DATA_DIR / "output_flux_spectrum.csv"
...
def tuning_artifact_paths() -> dict[str, Path]:
    FIGURES_DIR.mkdir(parents=True, exist_ok=True)
    REPORTS_DIR.mkdir(parents=True, exist_ok=True)
    DATA_DIR.mkdir(parents=True, exist_ok=True)
    return {
        "comparison_plot": FIGURES_DIR / "retuned_comparison.png",
        "recommendation_json": REPORTS_DIR / "retune_recommendation.json",
        "summary_json": REPORTS_DIR / "retune_summary.json",
    }
...
def run_retuned_simulation(
    ...
    tuned_summary = run_baseline_simulation(
        ...
        run_spec=BaselineRunSpec(
            artifact_stem="tuned_flux_spectrum",
            summary_stem="tuned_run_summary",
            plot_title="Retuned ring resonator transmission",
        ),
    )

    baseline_rows = load_spectrum_rows(csv_path)
    tuned_rows = load_spectrum_rows(DATA_DIR / "tuned_flux_spectrum.csv")
