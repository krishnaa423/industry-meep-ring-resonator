...
from meep_baseline_run import BaselineRunSpec, build_baseline_simulation
...
class FieldAnimationSpec:
    ...
    dpi: int = 120
    artifact_stem: str = "stage6_field_propagation"


def stage6_artifact_paths(artifact_stem: str) -> dict[str, Path]:
    FIGURES_DIR.mkdir(parents=True, exist_ok=True)
    REPORTS_DIR.mkdir(parents=True, exist_ok=True)
    return {
        "gif": FIGURES_DIR / f"{artifact_stem}.gif",
        "png": FIGURES_DIR / f"{artifact_stem}_preview.png",
...
def run_field_animation(
    ...
    paths = stage6_artifact_paths(animation_spec.artifact_stem)
    ...
    write_preview_png(
        destination=paths["png"],
        epsilon=epsilon,
        ez_field=frames[min(2, len(frames) - 1)],
...
def write_preview_png(
    ...
    axis.set_title("Field propagation preview")
    axis.set_xticks([])
    axis.set_yticks([])
    figure.tight_layout()
