...
from ring_layout import build_layout_geometry, write_layout_artifacts
...
def main() -> int:
    ...
    assert abs((bus_x_max - bus_x_min) - geometry.bus_length_um) < 1e-9
    assert abs((bus_y_max - bus_y_min) - geometry.bus_width_um) < 1e-9
    assert abs((ring_x_max - ring_x_min) - 2.0 * geometry.outer_ring_radius_um) < 0.05
    assert abs((ring_y_max - ring_y_min) - 2.0 * geometry.outer_ring_radius_um) < 0.05
    assert abs(0.5 * (ring_y_max + ring_y_min) - geometry.ring_center_y_um) < 0.05
